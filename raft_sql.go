...
func InitDB() *sql.DB {
	db, err := sql.Open("mysql", "root:jebasither@tcp(127.0.0.1:3306)/task_manager")
	if err != nil {
...
func main() {
	...
	http.HandleFunc("/api/failLeader", func(w http.ResponseWriter, r *http.Request) {
		leader := getRaftLeader()
		if leader != nil {
			var newLeaderID int
		for {
			newLeaderID = rand.Intn(3) // Generate a random number between 0 and 2
			if newLeaderID != leader.id {
				break
			}
		}
		fmt.Fprintf(w, "Leader (%d) failed. New leader nominated is %d\n", leader.id, newLeaderID)
		go nodes[newLeaderID].ElectLeader()

		} else {
			fmt.Fprint(w, "No leader found\n")
		}
	})

	http.HandleFunc("/api/currentLeader", func(w http.ResponseWriter, r *http.Request) {
		leader := getRaftLeader()
		if leader != nil {
			fmt.Fprintf(w, "Current leader is node %d\n", leader.id)
		} else {
			fmt.Fprint(w, "No leader found\n")
		}
	})
